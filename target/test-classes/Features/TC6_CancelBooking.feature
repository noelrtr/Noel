@CancelBooking
Feature: Cancel booking hotel in OMR Branch hotel page Automation

  Scenario Outline: Cancel order id in My Bookings
    ...
    Then User should verify success message after cancel the order id "Your booking cancelled successfully"
  ...
  Scenario Outline: Cancel order id in my bookings after editing the order id
    Given User is on the OMR Branch hotel page
    When User login "<userName>","<password>"
    Then User should verify success message after login "Welcome Vaijeyanthi"
    When User search hotels "<Select State>","<Select City>","<Select Room Type>","<Check-In>","<Check-Out>","<No. Of Room>","<No. Of Adults>" and "<No. Of Children>"
    ...
    When User navigates to My booking page and enter order id
    Then User should verify order id and hotel name are same
    When User edit the order id "<Change-Check-In>"
